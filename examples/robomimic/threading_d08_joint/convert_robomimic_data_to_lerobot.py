"""Convert every Threading_D08 absolute-joint demonstration to LeRobot.
...
DEFAULT_DATA_PATH = Path("/iris/u/tiangao/projects/robomimic/robomimic/datasets/threading_d08_joint/image_v15_256.hdf5")
DEFAULT_REPO_ID = "local/robomimic_threading_d08_joint_256"
DEFAULT_TASK_PROMPT = "Thread the needle through the ring"
DEFAULT_FILTER_KEY = "all"
EXPECTED_ENV_NAME = "Threading_D08"
EXPECTED_ENV_VERSION = "1.5.2"
EXPECTED_SOURCE_EPISODES = 300
EXPECTED_SOURCE_FRAMES = 97_077
...
def validate_environment_metadata(env_args: dict[str, Any]) -> int:
    """Validate the D08 task and its Panda absolute-joint recording schema."""
    ...
    # D08 changes task placement bounds and names its external view
    # ``agentview_full``. The D0 validator owns the common Panda/controller and
    # resolution checks after these task-specific aliases are normalized.
...
def validate_episode(demo_name: str, episode: h5py.Group) -> int:
    """Validate D08 schema and its audited absolute-joint semantics."""
    required = (
        "actions",
    ...
    # All 97,077 audited frames have target-current error <= 0.058283 rad,
    # and every episode's mean error decreases at the next state. This is
    # evidence for absolute radian targets, not normalized delta commands.
...
def convert_frame(episode: h5py.Group, index: int, task_prompt: str) -> dict[str, Any]:
    """Map D08's external camera name into the canonical training schema."""
...
def main(
    ...
    *,
    ...
    with h5py.File(source_path, "r") as source:
        ...
        for demo_name in tqdm(demos, desc="Converting Threading_D08 episodes"):
    ...
    if push_to_hub:
        dataset.push_to_hub(
            tags=["robomimic", "robosuite", "threading-d08", "panda", "joint-position"],
            private=False,
            push_videos=True,
            license="apache-2.0",
        )
