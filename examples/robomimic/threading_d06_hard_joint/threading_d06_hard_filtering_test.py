...
from examples.robomimic.threading_d06_hard_joint import build_filtering_keys as filtering
...
def test_balanced_sampling_is_deterministic_and_preserves_labels():
    indices = {"full": list(range(10, 20)), "partial": list(range(10))}

    sampled = filtering.sample_balanced_indices(indices, samples_per_label=3, seed=7)

    assert sampled == filtering.sample_balanced_indices(indices, samples_per_label=3, seed=7)
    assert len(sampled["sampled_full_only"]) == 3
    assert len(sampled["sampled_partial_only"]) == 3
    assert set(sampled["sampled_full_only"]) <= set(indices["full"])
    assert set(sampled["sampled_partial_only"]) <= set(indices["partial"])
    assert sampled["sampled_100"] == sorted(
        sampled["sampled_full_only"] + sampled["sampled_partial_only"]
    )


def test_balanced_sampling_rejects_too_few_candidates():
    with pytest.raises(ValueError, match="Cannot sample"):
        filtering.sample_balanced_indices({"full": [2], "partial": [0, 1]}, samples_per_label=2)
