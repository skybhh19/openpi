"""Build checked LeRobot episode filters from Threading_D06_Hard masks."""
...
conversion = importlib.import_module("examples.robomimic.threading_d06_hard_joint.convert_robomimic_data_to_lerobot")

DEFAULT_DATASET_PATH = Path("/iris/u/tiangao/lerobot_datasets/local/robomimic_threading_d06_hard_joint_256")
...
def sample_balanced_indices(
    ...
    *,
    samples_per_label: int = SAMPLED_EPISODES_PER_LABEL,
    seed: int = SAMPLED_SEED,
) -> dict[str, list[int]]:
    """Select one reproducible balanced subset and its full/partial components."""
    if samples_per_label <= 0:
        raise ValueError("samples_per_label must be positive")
    for label in ("full", "partial"):
        if len(indices[label]) < samples_per_label:
            raise ValueError(
                f"Cannot sample {samples_per_label} {label} episodes from only {len(indices[label])} candidates"
            )

    rng = random.Random(seed)
    sampled_partial = sorted(rng.sample(indices["partial"], samples_per_label))
    sampled_full = sorted(rng.sample(indices["full"], samples_per_label))
    return {
        "sampled_100": sorted(sampled_partial + sampled_full),
        "sampled_full_only": sampled_full,
        "sampled_partial_only": sampled_partial,
    }
...
def main(
    ...
    *,
    ...
    for mask_name, output_label in MASK_TO_OUTPUT_LABEL.items():
        output_path = destination / f"threading_d06_hard_joint_256_{output_label}_episode_indices.json"
    ...
    episode_rows = _read_jsonl(target_path / "meta" / "episodes.jsonl")
    episode_lengths = {int(row["episode_index"]): int(row["length"]) for row in episode_rows}
    sampled = sample_balanced_indices(indices)
    for output_label, selected in sampled.items():
        output_path = destination / f"threading_d06_hard_joint_256_{output_label}_episode_indices.json"
        output_path.write_text(json.dumps(selected, indent=2) + "\n")
        selected_frames = sum(episode_lengths[index] for index in selected)
        print(f"Wrote {output_path}: {len(selected)} episodes / {selected_frames} frames")
