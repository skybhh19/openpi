"""Build checked LeRobot episode filters from the 600-demo D06-harder masks."""
...
conversion = importlib.import_module(
    "examples.robomimic.threading_d06_harder_joint_600.convert_robomimic_data_to_lerobot"
)
...
def validate_mapping(source_path: Path, dataset_path: Path) -> tuple[list[str], dict[str, list[int]], dict[str, int]]:
    ...
    if manifest != expected_manifest:
        differing = sorted(
            key for key in set(manifest) | set(expected_manifest) if manifest.get(key) != expected_manifest.get(key)
        )
...
def main(
    ...
    *,
    ...
    for mask_name, output_label in MASK_TO_OUTPUT_LABEL.items():
        output_path = destination / f"threading_d06_harder_joint_600_256_{output_label}_episode_indices.json"
