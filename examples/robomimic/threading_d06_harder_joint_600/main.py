"""Evaluate an OpenPI policy from fresh Threading_D06_Harder resets."""
...
from examples.robomimic.threading_d06_harder_joint_600 import (  # noqa: E402
...
def make_environment_kwargs(
    ...
    *,
    ...
    """Preserve D06-harder's recorded task, controller, and camera identity."""
...
def _alias_agentview(observation: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    ...
    if "agentview_full_image" not in observation:
        raise KeyError("Threading_D06_Harder observation is missing agentview_full_image")
...
def run_rollout(env, policy, observation: dict[str, np.ndarray], **kwargs):
    """Run reconstructed absolute controller outputs with the full-camera alias."""
...
class Args(joint_evaluation.Args):
    ...
    output_path: str = "data/robomimic_threading_d06_harder_joint_600_256_eval.json"
