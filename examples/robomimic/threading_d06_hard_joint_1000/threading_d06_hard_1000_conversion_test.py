"""Focused tests for the Threading_D06_Hard absolute-joint converter."""
...
def test_environment_metadata_requires_d06_hard_absolute_joint_schema():
    assert conversion.validate_environment_metadata(_env_args()) == 20

    wrong_task = _env_args()
    wrong_task["env_name"] = "Threading_D06"
    with pytest.raises(ValueError, match="Threading_D06_Hard"):
...
def test_episode_accepts_audited_arm_step_and_maps_full_camera(tmp_path):
    ...
    with h5py.File(source_path, "w") as source:
        ...
        loaded_episode = conversion.load_episode(episode)

    # The bulk-loaded episode must be independent of the now-closed HDF5 file.
    frame = conversion.convert_frame(loaded_episode, 1, conversion.DEFAULT_TASK_PROMPT)
...
def test_real_source_metadata_and_complete_masks_are_stable():
    ...
    assert len(full) == len(partial) == 500
