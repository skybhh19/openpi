# Threading_D06_Hard 1,000-demo joint-position pipeline

This pipeline uses all 1,000 demonstrations (321,805 frames) in
`/iris/u/tiangao/projects/robomimic/robomimic/datasets/threading_d06_hard_joint_1000/image_v15_256.hdf5`.
...
./.venv/bin/python examples/robomimic/threading_d06_hard_joint_1000/convert_robomimic_data_to_lerobot.py
```
...
length, and resulting LeRobot episode index. It bulk-loads one episode at a
time so compressed HDF5 chunks are not repeatedly decompressed per frame.

For faster conversion, an exact local HDF5 copy may be passed with
`--data-path`; also pass the canonical Iris file via `--source-provenance-path`
so the manifest continues to identify and fingerprint the original source.

The distinct output is
`/iris/u/tiangao/lerobot_datasets/local/robomimic_threading_d06_hard_joint_1000_256`.
After conversion, rebuild the exclusive subset keys:

```bash
./.venv/bin/python examples/robomimic/threading_d06_hard_joint_1000/build_filtering_keys.py
```

Source `mask/full` becomes `full_only`; source `mask/partial` becomes
`partial_only`. Each contains 500 demos, they are disjoint, and they cover all
1,000 source demos.
...
./.venv/bin/python scripts/compute_norm_stats.py \
  --config-name pi05_robomimic_threading_d06_hard_joint_1000_low_mem_finetune
```

Training configs:

- `pi05_robomimic_threading_d06_hard_joint_1000_low_mem_finetune`
- `pi05_robomimic_threading_d06_hard_joint_1000_full_only_low_mem_finetune`
- `pi05_robomimic_threading_d06_hard_joint_1000_partial_only_low_mem_finetune`
- `pi05_robomimic_threading_d06_hard_joint_1000_30k_low_mem_finetune`
- `pi05_robomimic_threading_d06_hard_joint_1000_full_only_30k_low_mem_finetune`
- `pi05_robomimic_threading_d06_hard_joint_1000_partial_only_30k_low_mem_finetune`
...
```bash
./.venv/bin/python scripts/compute_norm_stats.py \
  --config-name pi05_robomimic_threading_d06_hard_joint_1000_absolute_low_mem_finetune
```

Absolute-action training configs:

- `pi05_robomimic_threading_d06_hard_joint_1000_absolute_low_mem_finetune`
- `pi05_robomimic_threading_d06_hard_joint_1000_absolute_full_only_low_mem_finetune`
- `pi05_robomimic_threading_d06_hard_joint_1000_absolute_partial_only_low_mem_finetune`
- `pi05_robomimic_threading_d06_hard_joint_1000_absolute_30k_low_mem_finetune`
...
The filtered absolute configs reuse the all-episode *absolute-action*
...
PYTHONPATH=/iris/u/tiangao/projects/robosuite:/iris/u/tiangao/projects/openpi \
  ./.venv/bin/python examples/robomimic/threading_d06_hard_joint_1000/main.py --help
```

## Tests

```bash
./.venv/bin/pytest -q examples/robomimic/threading_d06_hard_joint_1000
```
