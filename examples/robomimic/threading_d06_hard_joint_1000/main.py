"""Evaluate an OpenPI policy from fresh Threading_D06_Hard resets."""
...
from examples.robomimic.threading_d06_hard_joint_1000 import (  # noqa: E402
    convert_robomimic_data_to_lerobot as conversion,
)
...
def make_environment_kwargs(
    ...
    *,
    ...
    """Preserve D06-hard's recorded task, controller, and camera identity."""
    kwargs = _base_make_environment_kwargs(
        env_args,
        max_steps=max_steps,
        render_gpu_device_id=render_gpu_device_id,
        seed=seed,
        metadata_validator=metadata_validator or conversion.validate_environment_metadata,
    )
...
def _alias_agentview(observation: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    """Expose agentview_full under the shared evaluator's canonical key."""
    if "agentview_full_image" not in observation:
        raise KeyError("Threading_D06_Hard observation is missing agentview_full_image")
...
class _AgentviewAliasEnv:
    """Delegate to robosuite while aliasing step observations."""
...
def run_rollout(env, policy, observation: dict[str, np.ndarray], **kwargs):
    """Run the policy's absolute controller outputs with the task's full-camera alias."""
    return _base_run_rollout(_AgentviewAliasEnv(env), policy, _alias_agentview(observation), **kwargs)


joint_evaluation.validate_environment_metadata = conversion.validate_environment_metadata
...
class Args(joint_evaluation.Args):
    dataset_path: str = str(conversion.DEFAULT_DATA_PATH)
    output_path: str = "data/robomimic_threading_d06_hard_joint_1000_256_eval.json"
