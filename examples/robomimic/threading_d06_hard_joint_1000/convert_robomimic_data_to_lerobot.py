"""Convert every 1,000-demo Threading_D06_Hard joint demonstration to LeRobot.
...
maps the gripper to Pi closure. Training configs can either make the seven arm
dimensions relative or preserve them as absolute targets; the gripper remains
...
DEFAULT_REPO_ID = "local/robomimic_threading_d06_hard_joint_1000_256"
...
EXPECTED_SOURCE_EPISODES = 1_000
EXPECTED_SOURCE_FRAMES = 321_805
...
def validate_episode(demo_name: str, episode: h5py.Group) -> int:
    ...
    # Across all 321,805 audited frames, target-current error is <= 0.058385
...
def load_episode(episode: h5py.Group) -> dict[str, Any]:
    """Bulk-read one episode to avoid repeatedly decompressing HDF5 chunks per frame."""
    observation = episode["obs"]
    return {
        "actions": np.asarray(episode["actions"]),
        "obs": {
            SOURCE_AGENTVIEW_KEY: np.asarray(observation[SOURCE_AGENTVIEW_KEY]),
            "robot0_eye_in_hand_image": np.asarray(observation["robot0_eye_in_hand_image"]),
            "robot0_joint_pos": np.asarray(observation["robot0_joint_pos"]),
            "robot0_gripper_qpos": np.asarray(observation["robot0_gripper_qpos"]),
        },
    }


def convert_frame(episode: h5py.Group | dict[str, Any], index: int, task_prompt: str) -> dict[str, Any]:
...
def resolve_source_provenance_path(source_path: Path, source_provenance_path: str | None) -> Path:
    """Resolve the canonical source recorded when conversion reads an exact local staging copy."""
    if source_provenance_path is None:
        return source_path
    provenance_path = Path(source_provenance_path).expanduser()
    if not provenance_path.is_file():
        raise FileNotFoundError(provenance_path)
    if provenance_path.stat().st_size != source_path.stat().st_size:
        raise ValueError("Staged HDF5 and source provenance file sizes differ")
...
def main(
    ...
    *,
    ...
    with h5py.File(source_path, "r") as source:
        ...
        for demo_name in tqdm(demos, desc="Converting Threading_D06_Hard 1000 episodes"):
    ...
    if push_to_hub:
        dataset.push_to_hub(
            tags=["robomimic", "robosuite", "threading-d06-hard", "1000-demos", "panda", "joint-position"],
            private=False,
            push_videos=True,
            license="apache-2.0",
        )
