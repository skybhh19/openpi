"""Build LeRobot episode filters for a DROID wrench-on-hook dataset."""
...
def _validate_and_index_rows(
    ...
    for csv_position, row in enumerate(rows):
        ...
        ep_idx = int(row["ep_idx"]) if row.get("ep_idx") else episode_to_index[episode_name]
        expected_idx = episode_to_index[episode_name]
        if ep_idx != expected_idx:
            raise ValueError(
                f"CSV episode index mismatch for {episode_name!r}: row has ep_idx={ep_idx}, "
...
def build_filters(
    ...
    *,
    ...
    for pct in filter_pcts:
        ...
        label_filename = f"{file_prefix}_observabilitypct{pct}_episode_indices.json"
    ...
    manifest = {
        "observability": label_manifest,
        "random": random_manifest,
    }
    if not check_only:
        _write_json(output_dir / f"{file_prefix}_observability_filters_manifest.json", label_manifest)
