"""Build LeRobot episode filters for the June 20 wrench-on-hook dataset."""
...
ANNOTATIONS_CSV = Path(__file__).with_name("observability_scores_2026-06-20.csv")
OUTPUT_DIR = Path(__file__).with_name("lerobot_filtering_keys")
RAW_DATA_DIR = Path("/iris/u/tiangao/projects/droid/data/success/wrench-on-hook-0620")
LEROBOT_DATASET_DIR = Path("/iliad/u/tiangao/lerobot_datasets/skybhh19/droid_wrench_on_hook_06202026")

DATASET_NAME = "skybhh19/droid_wrench_on_hook_06202026"
FILE_PREFIX = "wrench_on_hook_06202026"
...
def _validate_annotations(rows: list[dict[str, str]], raw_paths: list[Path], lerobot_dataset_dir: Path) -> None:
    ...
    for expected_idx, (row, raw_episode_path) in enumerate(zip(rows, raw_paths, strict=True)):
        ...
        raw_episode_dir = raw_episode_path.parent
        if row["episode"] != raw_episode_dir.name:
            raise ValueError(
                f"CSV episode mismatch for ep_idx={ep_idx}: row has {row['episode']!r}, "
                f"raw path has {raw_episode_dir.name!r}"
            )

        csv_episode_path = Path(row["episode_path"])
        if csv_episode_path != raw_episode_dir:
            raise ValueError(
                f"CSV episode_path mismatch for ep_idx={ep_idx}: row has {csv_episode_path}, "
                f"raw path has {raw_episode_dir}"
            )
        if not (csv_episode_path / "trajectory.h5").exists():
            raise FileNotFoundError(f"Missing trajectory.h5 for ep_idx={ep_idx}: {csv_episode_path}")

        for camera_key in ("camera_0_video", "camera_1_video"):
            camera_path = Path(row[camera_key])
            if not camera_path.exists():
                raise FileNotFoundError(f"Missing {camera_key} for ep_idx={ep_idx}: {camera_path}")
        ...
        try:
            observability = int(row["observability_score"])
        except ValueError as exc:
            raise ValueError(
                f"Invalid observability score for ep_idx={ep_idx}: {row['observability_score']!r}"
            ) from exc
...
def _select_by_observability(rows: list[dict[str, str]], pct: int) -> list[int]:
    ...
    ranked_rows = sorted(rows, key=lambda row: (-int(row["observability_score"]), int(row["ep_idx"])))
...
def build_filters(
    ...
    observability_manifest = {
        ...
        "selection": {
            ...
            "rank": "higher numeric observability_score is better",
            "tie_break": "ascending ep_idx within the same score",
        },
        "label_counts": {"observability_score": dict(Counter(row["observability_score"] for row in rows))},
        "filters": {},
    }
    ...
    for pct in FILTER_PCTS:
        ...
        observability_manifest["filters"][observability_filename] = {
            ...
            "label_counts": dict(
                Counter(rows[index]["observability_score"] for index in observability_indices)
            ),
