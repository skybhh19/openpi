...
def main(
    ...
    *,
    repo_name: str,
    language_annotation: str = "Do something",
    push_to_hub: bool = False,
):
    """
    Convert DROID-style trajectories to LeRobot. By default the output
    directory is the same as data_dir; 
    """
    ...
    #     - trajectory.hdf5
    #   - <...>/
    episode_paths = list(data_dir.glob("**/traj.hdf5"))
    ...
    for episode_path in tqdm(episode_paths, desc="Converting episodes"):
        ...
        # All episodes use the provided language_annotation string
        language_instruction = language_annotation
        print(f"Converting episode with language instruction: {language_instruction}")

        # Write to LeRobot dataset
        for step in trajectory:
            # print(step["saved_observation"]["images"]["exterior_image_1_left"].shape)
            # print(step["action"])
            dataset.add_frame(
                {
                    # Note: need to flip BGR --> RGB for loaded images
                    # Already flipped in the saved_observation
                    "exterior_image_1_left": resize_image(
                        step["saved_observation"]["exterior_image_1_left"], (320, 180)
                    ),
                    # TODO: only use one side camera, droid finetuning only uses one side camera
                    # randomly, not using the right side camera
